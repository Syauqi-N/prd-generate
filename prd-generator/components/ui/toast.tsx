...
>(({ className, variant = "default", ...props }, ref) => {
  const variantStyle: React.CSSProperties =
    variant === "destructive"
      ? { backgroundColor: "rgba(239,68,68,0.1)", borderColor: "rgba(239,68,68,0.3)", color: "#ef4444" }
      : variant === "success"
      ? { backgroundColor: "color-mix(in srgb, var(--accent) 10%, var(--bg-card))", borderColor: "color-mix(in srgb, var(--accent) 30%, transparent)", color: "var(--accent)" }
      : { backgroundColor: "var(--bg-card)", borderColor: "var(--border-color)", color: "var(--text-primary)" }

  return (
    <ToastPrimitive.Root
      ref={ref}
      className={cn("group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-4 pr-8 shadow-lg transition-all", className)}
      style={variantStyle}
      {...props}
    />
  )
